...
import { openClinicalView } from './clinical/clinicalView.js';
import { waitForNext } from './ui/nextButton.js';
import { startPlaythrough, commitPlaythrough } from './clinical/sessionRecorder.js';
...
function showIntro() {
  app.innerHTML = `
    <div id="preloader">
      <div id="preloader-text">FixedGap</div>
    </div>
    <div id="intro">
      <div class="intro-content">
        <h1 class="intro-title">FixedGap</h1>
        <p class="intro-subtitle">Motor Rehabilitation Telemonitoring</p>
        <p class="intro-desc">Interactive exercises for patients with stroke, Parkinson's or multiple sclerosis</p>
        <button id="start-demo-btn" class="star-button">
          <div class="star-btn-light"></div>
          <div class="star-btn-border" aria-hidden="true">
  ...
  function animatePreloaderOut() {
    ...
    tl.to(preloaderText, { scale: 5, opacity: 0, duration: 0.8 });
    tl.to(preloader, {
      y: '-105%',
    ...
    tl.to(intro, { opacity: 1, duration: 0.6 }, '-=0.4');
  }

  checkReady();

  const btn = document.getElementById('start-demo-btn');
  const light = btn.querySelector('.star-btn-light');

  function measureButton() {
    const w = btn.offsetWidth;
    const h = btn.offsetHeight;
    if (w > 0 && h > 0) {
      light.style.offsetPath = `path('M 0 0 H ${w} V ${h} H 0 V 0')`;
    } else {
      requestAnimationFrame(measureButton);
    }
  }
  requestAnimationFrame(measureButton);
...
function showLoadingScreen() {
  const cleanup = showTetrisLoader(app);
  return cleanup;
}

function showBehindTheScenes() {
  return new Promise((resolve) => {
    app.innerHTML = `<div id="bts-screen"><p class="bts-text">Behind the Scenes</p></div>`;
    const screen = document.getElementById('bts-screen');
    gsap.fromTo(screen, { x: '100%' }, { x: '0%', duration: 0.5, ease: 'power2.out' });
    setTimeout(resolve, 2500);
  });
}

function showClinicalInterlude() {
  return new Promise((resolve) => {
    const cleanup = openClinicalView(app, () => {});
    setTimeout(() => {
      if (cleanup) cleanup();
      resolve();
    }, 7000);
...
async function runDemo() {
  ...
  for (let i = 0; i < DEMO_SEQUENCE.length; i++) {
    ...
    await waitForNext(isLast ? 'Ver informe' : 'Siguiente juego');

    if (cleanup) cleanup();
  }

  // Persist this playthrough as a new session in Alpha's history (localStorage).
  commitPlaythrough('right');

  const cleanupLoader = showLoadingScreen();
